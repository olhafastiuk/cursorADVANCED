...
document.forms.form_1.onsubmit = function () {
    ...
    if (maxNumber != undefined) {
        document.getElementById(
            "output-1"
        ).innerHTML = `Найбільше цифра у введеному числі: ${maxNumber}`;
    }
    return false;
};
...
document.getElementById("input-2").addEventListener("click", () => {
    ...
    const girls = students.filter(
        (student) => student[student.length - 1] === "а"
    );
    const boys = students.filter(
        (student) => student[student.length - 1] !== "а"
    );
    const itemsPairs = getPairs(boys, girls)
        .map((item) => item.join(", "))
        .join(";<br>");
...
document.getElementById("input-3").addEventListener("click", () => {
    ...
    if (minNumber && maxNumber && len) {
        document.getElementById(
            "output-3"
        ).innerHTML = `Новий масив з випадкових чисел: ${getRandomArray(
            len,
            minNumber,
            maxNumber
        ).join(", ")}`;
...
document.getElementById("input-4").addEventListener("click", () => {
    document.getElementById(
        "output-4"
    ).innerHTML = `Середній бал: ${getAverageMark(students[0])}`;
});
...
document.forms.form5.onsubmit = function () {
    ...
    document.getElementById(
        "output-5"
    ).innerHTML = `Ви заплатите податків на таку суму: <span>${getMyTaxes(
        country.tax,
        sal
    )} $</span>`;
    return false;
